...
# Make SSE request and show streaming response (with gtimeout for macOS)
echo "SSE Stream Trace (llm.token → llm.done):"
echo "----------------------------------------"

# Use gtimeout if available, otherwise use a simple approach
if command -v gtimeout >/dev/null 2>&1; then
    gtimeout 30s curl -s -X POST "http://localhost:8081/v1/chat/stream" \
        -H "Content-Type: application/json" \
    ...
    # Fallback: use a background process with sleep
    (
    ...
    # Wait for 30 seconds then kill the background process
    sleep 30
